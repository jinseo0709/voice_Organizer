...
// 카테고리 정의
var VoiceMemoCategory;
(function (VoiceMemoCategory) {
    VoiceMemoCategory["SHOPPING"] = "\uC1FC\uD551\uB9AC\uC2A4\uD2B8";
    VoiceMemoCategory["TODO"] = "\uD22C\uB450\uB9AC\uC2A4\uD2B8";
    VoiceMemoCategory["APPOINTMENT"] = "\uC57D\uC18D \uC77C\uC815";
    VoiceMemoCategory["HOMEWORK"] = "\uD559\uAD50 \uC218\uC5C5 \uACFC\uC81C \uC77C\uC815";
    VoiceMemoCategory["IDEA"] = "\uC544\uC774\uB514\uC5B4";
    VoiceMemoCategory["OTHER"] = "\uAE30\uD0C0";
})(VoiceMemoCategory || (exports.VoiceMemoCategory = VoiceMemoCategory = {}));
class LanguageAnalysisService {
        ...
        try {
            ...
            if (options.enableEntityAnalysis !== false) {
                try {
                    ...
                    results.entities = entityResponse.entities?.map((entity) => ({
                        name: entity.name,
                        type: entity.type,
            ...
            if (options.enableSentimentAnalysis !== false) {
                try {
                    ...
                    results.sentiment = {
                        ...
                        sentences: sentimentResponse.sentences?.map((sentence) => ({
            ...
            if (options.enableClassification !== false) {
                try {
                    ...
                    results.classifications = classificationResponse.categories?.map((category) => ({
                        name: category.name,
    ...
    // 모의 구현
    async mockAnalyzeText(text, options = {}) {
    ...
    // 카테고리별 맞춤형 요약 생성
    async generateCategorizedSummary(text) {
    ...
    categorizeAndSummarize(text, analysisResult) {
        // 키워드 추출
        const keywords = [];
    ...
    // 모의 카테고리 분석
    mockGenerateCategorizedSummary(text) {
        const category = this.determineCategory(text);
        const summary = this.generateSummaryForCategory(text, category);
    ...
    generateSummaryForCategory(text, category) {
        const maxLength = 100;
        switch (category) {
    ...
    determinePriority(text, analysisResult) {
