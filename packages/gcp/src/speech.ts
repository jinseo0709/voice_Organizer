...
// 동적 import를 위한 타입 정의
type ISpeechRecognitionResult = any;
type IRecognizeRequest = any;
...
export class SpeechToTextService {
  ...
  // 공통 결과 파싱
  private parseRecognitionResult(response: any): TranscriptionResult {
    ...
    return {
      ...
      wordTimeOffsets: alternative.words?.map((word: any) => ({
        word: word.word || '',
  ...
  // 모의 구현
  private async mockTranscribeAudio(
    audioBuffer: Buffer,
    options: TranscriptionOptions = {}
  ): Promise<TranscriptionResult> {
    const audioSizeKB = audioBuffer.length / 1024;
