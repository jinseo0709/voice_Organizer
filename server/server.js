...
app.post('/api/gemini-analysis', async (req, res) => {
  try {
    ...
    // 오늘 날짜 정보 (AI가 상대적 날짜를 계산할 수 있도록)
    const today = new Date();
    const todayStr = `${today.getFullYear()}년 ${today.getMonth() + 1}월 ${today.getDate()}일`;
    const dayOfWeek = ['일', '월', '화', '수', '목', '금', '토'][today.getDay()];

    // 🎯 강화된 AI 분석 프롬프트 (카테고리 분리 강제)
    const prompt = `[시스템 역할]
당신은 사용자의 음성 메모 텍스트를 분석하여 6가지 지정된 카테고리(쇼핑리스트, 투두리스트, 약속 일정, 학교 수업 과제 일정, 아이디어, 기타)로 분류하고, 각 카테고리별로 추출된 핵심 정보를 구조화하여 오직 JSON 배열 형태로 반환하는 최고 등급의 데이터 프로세서입니다.

📅 오늘 날짜: ${todayStr} (${dayOfWeek}요일)

[핵심 임무: 카테고리 분리 및 항목화 강제]
입력된 텍스트에는 복수의 카테고리 내용이 혼재되어 있을 수 있습니다.
당신의 임무는:
1. 해당되는 모든 카테고리를 정확히 식별
2. 각 카테고리 내용을 별도의 객체로 분리하여 JSON 배열에 포함
3. 원본 텍스트를 그대로 복사하지 않고, 반드시 요약 및 항목화 규칙을 준수
4. 하나의 카테고리 내에 여러 개의 할 일이나 물품이 언급되었다면, 각각을 독립된 항목으로 분리하여 summary_list 배열에 포함
...
🛒 쇼핑리스트:
   - 구매할 물품만 추출하여 개별 항목으로 분리
   - 명사 형태만 사용 (수량 포함 시 함께 기재)
   - 예시: ["바나나 2송이", "커피믹스", "에어팟 케이스 1개", "우유 1L"]

✅ 투두리스트:
   - 수행해야 할 행동을 동사형 완료 형태로 추출하여 개별 항목으로 분리
   - 날짜/시간이 없는 일반적인 할 일만 포함
   - 예시: ["아르바이트 정산서 처리하기", "학생회비 입금하기", "방 청소하기", "이메일 답장하기"]

📅 약속 일정:
   - 날짜, 시각, 장소, 내용을 추출하여 캘린더 등록에 적합한 하나의 문장 형식으로 재구성
   - 상대적 날짜(내일, 모레, 이번 주 목요일 등)는 실제 날짜로 변환
   - 형식: "YYYY년 MM월 DD일 [시간]에 [장소]에서 [내용]"
   - 예시: ["2025년 12월 14일 오후 2시에 홍대입구역에서 언니와 전시회 관람", "2025년 12월 20일 저녁 7시에 강남역에서 친구와 저녁 식사"]

🎓 학교 수업 과제 일정:
   - 과제명과 제출/마감 기한 정보를 명확히 포함하여 개별 항목으로 분리
   - 형식: "YYYY년 MM월 DD일 [시간]까지 [과제명] 제출"
   - 예시: ["2025년 12월 11일 오후 3시까지 설문조사 분석 PPT 제출", "2025년 12월 15일 자정까지 프로그래밍 과제 제출"]

💡 아이디어:
   - 핵심 아이디어를 구조화된 텍스트로 요약
   - summary_list: "제목: [제목] / 배경: [배경] / 핵심 구상: [내용]" 형태
   - ai_supplement: Gemini가 이 아이디어에 대해 보충한 의견이나 구체화 제안 (2~4문장, 필수)
   - 예시:
     summary_list: ["제목: 스마트 물병 / 배경: 물 마시기 습관이 안 됨 / 핵심 구상: 음수량 추적 및 알림 기능"]
     ai_supplement: "이 아이디어는 건강 관리 앱과 연동하면 시너지가 클 것입니다. 온도 센서를 추가하면 더 유용할 수 있습니다."

📝 기타:
   - 위 5개 카테고리에 해당하지 않는 내용
   - 2줄 이내로 간결하게 요약
   - 예시: ["오늘 날씨가 좋아서 기분이 좋았다는 일상 기록"]

[필수 출력 형식]
오직 아래 구조의 JSON 배열만 출력하세요. 어떠한 설명이나 추가 텍스트도 포함하지 마세요.

[
  {
    "category": "쇼핑리스트",
    "summary_list": ["물품명 수량", "물품명"],
    "keywords": ["장보기", "구매"]
  },
  {
    "category": "투두리스트",
    "summary_list": ["할일1 하기", "할일2 처리하기"],
    "keywords": ["할일", "업무"]
  },
  {
    "category": "학교 수업 과제 일정",
    "summary_list": ["2025년 MM월 DD일 시간까지 과제명 제출"],
    "keywords": ["과제", "제출", "마감"]
  },
  {
    "category": "약속 일정",
    "summary_list": ["2025년 MM월 DD일 시간에 장소에서 누구와 무엇"],
    "keywords": ["약속", "일정", "만남"]
  },
  {
    "category": "아이디어",
    "summary_list": ["제목: ... / 배경: ... / 핵심 구상: ..."],
    "keywords": ["아이디어", "구상"],
    "ai_supplement": "Gemini가 이 아이디어에 대해 보충한 의견 (2~4문장)"
  },
  {
    "category": "기타",
    "summary_list": ["해당 내용 2줄 이내 요약"],
    "keywords": ["기타"]
  }
]

⚠️ 절대 준수 사항:
1. 해당 카테고리에 내용이 없으면 해당 카테고리 객체를 출력하지 마세요
2. 시간/날짜가 포함된 항목 중 과제는 "학교 수업 과제 일정", 개인 일정은 "약속 일정"으로 분류
3. 각 항목은 반드시 개별 항목으로 분리 (하나의 문자열에 여러 항목을 절대 넣지 않음)
4. 아이디어 카테고리가 있을 경우 반드시 ai_supplement 필드를 포함
5. JSON 배열만 출력하고 다른 설명이나 마크다운은 절대 포함하지 마세요
6. 상대적 날짜는 반드시 오늘 날짜(${todayStr})를 기준으로 실제 날짜로 변환`;
