...
export async function POST(request: NextRequest) {
  try {
    ...
    // 텍스트 분석 프롬프트 - 카테고리를 앱의 UI와 일치시킴
    const prompt = `
다음 음성 메모 텍스트를 종합적으로 분석해주세요:

텍스트: "${text}"

분석 항목:
1. 카테고리 분류 (다음 중 가장 적합한 것 하나 선택):
   - "쇼핑리스트": 구매할 물건, 장보기 목록 관련
   - "투두리스트": 할 일, 업무, 작업 목록 관련
   - "약속 일정": 만남, 약속, 일정, 예약 관련
   - "학교 수업 과제 일정": 학교, 수업, 과제, 시험, 학습 관련
   - "아이디어": 아이디어, 영감, 메모, 생각 기록
